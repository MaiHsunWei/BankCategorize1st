...
// A route for POST requests sent to `/findBankIDBankName`
app.post("/findBankIDBankName", function(req, res) {
  if (!req.body.bankName) {
    console.log("No BankName : " + JSON.stringify(req.body));
    return res.send({ status: "error", message: "missing parameter(s)" });
  } else {
    console.log("Received POST: " + JSON.stringify(req.body));
	console.log(findBankIDBankName(req.body.bankName));
	
    return res.send(findBankIDBankName(req.body.bankName));
  }
});
